...
public static class MarkdownSteam
{
    public static string ToBBCode(string markdown)
    {
        ...
        var renderer = new BBCodeRenderer(BBCodeType.Steam, pipeline, false, false, writer) { Source = markdown };
    ...
    public static string ToBBCodeExtended(string markdown)
    {
        ...
        using var writer = new StringWriter();
        var renderer = new BBCodeRenderer(BBCodeType.Steam, pipeline, true, true, writer) { Source = markdown };
        renderer.Render(document);
        renderer.Writer.Flush();
